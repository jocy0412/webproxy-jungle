...
// 1. 입력 : ./tiny 4500
// 2. aws : 3.35.220.147:4500 접속

#include "csapp.h"

void doit(int fd); // 연결된 client에 대해서 한 HTTP 트랜잭션 처리
void echo(int connfd);
void read_requesthdrs(rio_t *rp); // 불필요한 request header 부분 무시
int parse_uri(char *uri, char *filename, char *cgiargs); // URI를 파싱하여 정적 컨텐츠인지 동적 컨텐츠인지 확인
void serve_static(int fd, char *filename, int filesize, char *method); // 정적 컨텐츠 처리
void get_filetype(char *filename, char *filetype); // file의 type을 확인하여 filetype에 저장
void serve_dynamic(int fd, char *filename, char *cgiargs, char *method); // 동적 컨텐츠 처리
void clienterror(int fd, char *cause, char *errnum, char *shortmsg, char *longmsg); // 에러에 대해서 클라이언트에게 응답
...
void doit(int fd) { /* 한 개의 HTTP 트랜잭션을 처리 */
  ...
  is_static = parse_uri(uri, filename, cgiargs); // URI 파싱하여 파일 이름과, 비어있을 수도 있는 CGI인자 스트링 분석하고
  ...
  if (stat(filename, &sbuf) < 0) { // 요청이 정적 또는 동적 컨텐츠를 위한 것인지 나타내는 플래그 설정 // 버퍼에 파일네임을 넘긴다
    ...
    // 만일 파일이 디스크 상에 없으면, 에러메시지를 즉시 클라이언트에게 보내고 리턴.
    return;
  }
  ...
  if (is_static) { /* Serve static content */ // 만일 요청이 정적 컨텐츠를 위한 것이라면
    ...
    serve_static(fd, filename, sbuf.st_size, method); // 맞다면 정적 컨텐츠를 클라이언트에게 제공
  }
  else { /* Serve dynamic content */
    if (!(S_ISREG(sbuf.st_mode)) || !(S_IXUSR & sbuf.st_mode)) { // 동적 컨텐츠에 대한 것이라면 이 파일이 실행 가능한지 검증
...
void clienterror(int fd, char *cause, char *errnum, char *shortmsg, char *longmsg) {
  ...
  Rio_writen(fd, buf, strlen(buf));
  Rio_writen(fd, body, strlen(body)); // 위에서 작성된 response body 아래에 붙이기
}

/* 불필요한 request header 부분 무시 */
...
// URI에 filename과 cgiargs를 파싱하고, 정적인지 동적인지 판단하여 리턴하는 함수
int parse_uri(char *uri, char *filename, char *cgiargs) {
  ...
  if(!strstr(uri, "cgi-bin")) { /* Static content */ // 만일 요청이 정적 컨텐츠를 위한 것이라면
    ...
    if (uri[strlen(uri)-1] == '/') // URI가 '/'로 끝나면
      strcat(filename, "home.html"); // 기본 파일 이름(home.html)을 filename에 추가
    return 1;
  }

  // uri 예시: dynamic: /cgi-bin/adder?first=1213&second=1232
  else { /* Dynamic content */ // 반면, 만일 이 요청이 동적 컨텐츠를 위한 것이라면
    ptr = index(uri, '?'); // 모든 CGI 인자들을 추출, index 함수는 문자열에서 특정 문자의 위치를 반환
...
void serve_static(int fd, char *filename, int filesize, char *method) {
  ...
  get_filetype(filename, filetype); // filname 검사해서 파일 타입 결정 return해줄 content-type 세팅
  sprintf(buf, "HTTP/1.0 200 OK\r\n"); // 클라이언트에 응답 줄과 응답 헤더를 보냄
  sprintf(buf, "%sServer: Tiny Web Server\r\n", buf);
  sprintf(buf, "%sConnection: close\r\n", buf);
  sprintf(buf, "%sContent-length: %d\r\n", buf, filesize);
  sprintf(buf, "%sContent-type: %s\r\n\r\n", buf, filetype); // 빈줄 한 개로 헤더를 종료
